...
import { FaArrowRight, FaCar, FaCheck, FaKey, FaMapMarkerAlt, FaPhoneAlt, FaShieldAlt } from 'react-icons/fa'
...
const PriceCard = ({ vehicle }: { vehicle: Vehicle }) => <aside className="h-fit bg-[var(--color-primary)] p-5 text-[var(--color-surface)] sm:p-6 lg:sticky lg:top-24 lg:self-start lg:p-7">
  ...
  <div className="mt-6 border-t border-white/20 pt-5"><ul className="grid gap-4 text-sm"><li className="flex items-center gap-3"><FaCar className="w-4 shrink-0 text-[var(--color-accent)]" />{vehicle.status || 'Available now'}</li><li className="flex items-center gap-3"><FaKey className="w-4 shrink-0 text-[var(--color-accent)]" />Clean title</li><li className="flex items-center gap-3"><FaShieldAlt className="w-4 shrink-0 text-[var(--color-accent)]" />Quality inspected</li><li className="flex items-center gap-3"><FaMapMarkerAlt className="w-4 shrink-0 text-[var(--color-accent)]" />Local pickup in Dallas, PA</li></ul></div>
</aside>
